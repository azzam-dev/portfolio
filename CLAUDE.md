...
- **لا تُنشئ صفحة جديدة لقسم محتوى.** الموقع صفحة واحدة: القسم الجديد يُضاف إلى
  `index.astro` بـ`id` وبثابت `panel` نفسه (شاشة كاملة + التقاط)، ورابطه في
  `Header.astro` عبر `section("<id>")`. الاستثناء الوحيد هو صفحتا دراسة الحالة.
...
- **قوالب الأقسام مشتركة:** `.eyebrow` و`.chip` و`.btn-accent` و`.link-row` في
